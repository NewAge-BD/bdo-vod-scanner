...
- Provide one drag-and-drop surface accepting exactly one supported event source and any number of MP4 VODs. Supported event sources are canonical `YYYY-MM-DD.log`, descriptive `.log` filenames containing an ISO or `DD.MM.YYYY` date, GuildYapper text lines using `killed`, `has killed`, or `died to`, and validated Ikusa `raw-session` JSON v4 files ending in `.ikusa.json`. Include a collapsible four-step GuildYapper download guide beside the importer.
- Let users rename each VOD's persisted display name from the imported-sources card without changing its immutable source filename or duplicate-detection identity.
...
Use the available width for the complete synchronization workspace and keep all instructional copy, labels, timestamps, event rows, controls, and status text in the right-hand synchronization panel comfortably readable at desktop widths.

Render synchronization event text with accessible colors inspired by BDO's in-game system chat: family names in gold, kill verbs and guild names in cyan, only the word **slain** in warm red, and structural details in a legible neutral grey. Phrase deaths as **Family was slain by Family**, keeping **was** and **by** in the same neutral color as event timestamps, and retain **from Guild** in the detailed selected event. Apply the same semantic coloring to the selected event and the matching-event list without relying on color alone to convey the event verb.

Offer an experimental **Auto Sync** helper before manual confirmation. The user draws one normalized rectangle around the in-game chat for the active VOD without resetting the current image zoom or pan. A locally bundled Tesseract.js worker samples only that crop, beginning near the current playhead and expanding outward in bounded intervals. Match recognized `killed` chat lines against log character/family names, opposing guild, verb, and the visible chat timestamp; require both opposing names and an exact matching hour and minute before proposing an anchor. Support a wrapped standalone timestamp immediately after its kill line. After a coarse hit, expand the search backward until the matched line is absent, then use sub-second binary refinement to find the first readable frame where the fresh chat message appears. Tell users that recognition works much better when they use BDO's in-game crop mode and place the chat in its grey area. Show progress, cancellation, a cropped preview, matched log line, and confidence. A successful result immediately seeks the main player to the proposed frame. Then explicitly ask the user to step backward or forward frame by frame until reaching the first frame where the kill notification appears before confirming the normal synchronization point. Keep the region, OCR text, and preview session-only, discard them when the workspace is closed, never upload them, and preserve manual synchronization as the fallback.
...
In the clipping workspace, show a compact full-recording filmstrip beneath the zoomed video timeline. Highlight the currently visible time window and let users drag it across the full duration without changing the zoom level. Keep the paused playhead centered in that window while it moves. Provide equivalent arrow-key navigation plus Home and End controls.

Keep synchronization and clip selection as separate workspaces. A **Start Clipping** action below the synchronization confirmation opens the clipping workspace with the active perspective as a large single player and its timeline across the full available width. Offer an optional split-screen view in the clipping controls that shows every synchronized perspective whose local VOD is linked for the current session; keep the active perspective as the timeline authority and provide an individual mute toggle for every perspective. Use the wider desktop canvas, a viewport-aware player height, readable event lanes, and pointer-friendly markers and clip handles. While clipping, hide the project heading, project navigation, source import, and imported-source cards above the workspace. Restore them when the user returns to synchronization.

Do not create extra timeline rows for additional VOD perspectives. Show shared **Kills** and **Deaths** rows, followed by a **Selected names** text entry. Align the video playhead track and every event track to the same horizontal time scale after accounting for the event-label column. Adding a family, character, or guild name creates one independently filtered event timeline for that name; users may add or remove as many name timelines as needed within the saved per-perspective limit. Give each name timeline a compact split toggle that changes its combined row into separate kill and death rows and can merge them again. Persist that layout per VOD in the project. Support mouse-wheel zoom anchored to the exact time beneath the pointer and horizontal navigation. Bundle dense events while zoomed out and separate them while zoomed in. Render `killed` green, `died to` red, and ordinary mixed bundles neutrally. Only user-created name timelines summarize 15-second kill bursts using the supplied in-game hierarchy: two kills as gold/orange **I: Challenger!**, three as red **II: Invader!**, four as violet **III: Slayer!**, and five or more as pink/magenta **IV: Conqueror**. Use one banner spanning from the first through the final kill instead of overlapping one badge per threshold, and place nearby ordinary events on a separate visual level. Measure the rendered track width and collapse every banner that would horizontally overlap another banner into its diamond emblem; expand it again automatically when timeline zoom provides sufficient room. Center the tier's Roman numeral inside the diamond. If a name timeline is split, keep the streaks on its kill row only. The shared synchronization log and the fixed clipping **Kills** and **Deaths** rows retain ordinary markers and neutral bundles. Recreate the compact banner, glow, strictly centered serif italic title, and diamond emblem in accessible CSS instead of embedding screenshots. Use efficient graphics for dense markers plus a semantic keyboard-operable event list. Clicking an individual or bundled marker selects a representative event, seeks, centers the timeline, and pauses. Kill-streak markers always select their first kill. In the clipping workspace, selecting an ordinary event prepares an editable range from ten seconds before through ten seconds after it. Selecting a combined kill-streak event starts ten seconds before its first kill and ends ten seconds after its final kill. Clamp every suggested range to the recording. Display clip ranges with draggable handles. Render a resource-conscious local thumbnail filmstrip for the visible video timeline window without loading the entire VOD into memory. Keep video controls in a dedicated bar outside the picture so edge-aligned kill feeds and chat remain unobstructed.
