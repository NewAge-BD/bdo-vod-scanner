...
The guided tour is a presentation-only layer rendered at the document root so it remains independent from header and workspace layout contexts. Stable `data-guide` hooks identify contextual targets without coupling workflow components to tour state. The tour observes only DOM layout changes, never performs application actions, and stores one versioned completion flag in `localStorage`; project data continues to live exclusively behind the project repository.
